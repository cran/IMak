...
#' case, the target object should have the same name as the
...
#'
#' @param items An object of class \code{'fa_items'} generated with function \code{build_fa}.
#' @param which A numeric vector designating which isomorph(s) to plot. Plot all by defect.
#' @param mode A character string designating plot mode "A", "B" or "C". Plot mode "A" by defect.
#' @param language A character string designating English ("E"), German ("D") or Spanish ("S") language. "E" by defect.
#' @param language.dir A character string designating language for output files. "A" by defect selects all languages.
#' @param form.int A character string designating the form from "A" to "D" of the internal shape lines, or "R" for random. "A" by defect.
#' @param form.ext A character string designating the form from "A" to "D" of the external trapezium, or "R" for random. "A" by defect.
#' @param size.shape A number designating the size of every shape. 1 by defect.
#' @param size.dot A number designating the size of every shape dot. 2 by defect.
#' @param size.line A number designating the thickness of every shape line. 1 by defect.
#' @param size.q A number designating the size of the question mark. 3.5 by defect.
#' @param size.word A number designating the size of the verbal options. 1.2 by defect.
#' @param info Should rules applied and correct answers be informed? True by defect.
...
#' @return A data frame containing rules applied and right answers when \code{info=T} by defect, or an object of class \code{'fa_items'} when \code{which} has length 1, its value is greater than 0 and both \code{switch.from} and \code{switch.to} are greater than 0.
...
#' \dontrun{Plot them:}
#' plot_fa(one)
#' \dontrun{Change the correct answer of item 2 from position 1 to position 2:}
#' one <- plot_fa(one, which = 2, switch.from = 1, switch.to = 2)
#' \dontrun{Choose a directory and save the items:
...
#' \dontrun{Plot them in German language:}
#' plot_fa(two,language = "D")
#' \dontrun{Plot only items 2 and 3 in Spanish and choose form "B" for the internal lines:}
#' plot_fa(two, language = "S", form.int = "B", which = c(2, 3))
#' \dontrun{Choose a different directory and save these two items by keeping the latter configuration:
...
#' \dontrun{Plot them:}
#' plot_fa(three)
#' \dontrun{Plot each individual shape of item 13 in German language only:}
#' plot_fa(three, which = 13, mode = "C", language = "D")
#' \dontrun{Choose a different directory and save the item parts:
...
#' @export
plot_fa <- function(
  items,              #List containing items already built.
  which = 0,          #0: plot all. Numbers > 0: item(s) chosen to ploy.
  mode = "A",         #A, B, C: different plot types.
  language = "E",     #E, D, S: English, Deutsch, Spanish.
  language.dir = "A", #A: All languages are printed. E, D, S: A specific language is printed.
  form.int = "A",     #A, B, C, D: different internal line types. R: random.
  form.ext = "A",     #A, B, C, D: different trapezium types. R: random.
  size.shape = 1,     #Size of every shape.
  size.dot = 2,       #Size of the dot.
  size.line = 1,      #Thickness of the lines.
  size.q = 3.5,       #Size of the question mark.
  size.word = 1.2,    #Size of options 9 and 10.
  info = T,           #Print item information in directory.
  directory = F,      #Choose a directory to save in png format.
  switch.from = 0,    #Option p to be switched with q.
  switch.to = 0)      #Option q to be switched with p.
